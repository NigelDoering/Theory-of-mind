...
def train_pipeline(
    ...
    # Fix device logic to handle gpu=-1 case
    ...
    if os.path.exists(graph_cuda_path):
        ...
        import time
        t0 = time.time()
        graph_data = data_loader.prepare_graph_data(device=device)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (trajs, target_goals, true_goals, ts) in enumerate(train_loader):
            ...
            loss = goal_loss + beta_kl * kl_loss + beta_recon * recon_loss
        ...
        if (epoch + 1) % 2 == 0:
            ...
            with torch.no_grad():
                for val_trajs, val_target_goals, val_true_goals, val_ts in val_loader:
                    ...
                    val_loss_total = goal_loss + beta_kl * kl_loss + beta_recon * recon_loss
                    ...
                    val_loss += val_loss_total.item() * val_target_goals.size(0)
    ...
    best_model_save_path = f"./trained_models/{model_save_name}_best.pth"
